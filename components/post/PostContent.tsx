...
const PostContent: React.FC<PostContentProps> = ({ post, comments, deleteComment, userId, likes, toggleLike, userLikes, showLoginPopup, setShowLoginPopup }) => {
  ...
  const [likeDetails, setLikeDetails] = useState<{ id: string; username: string; avatar_url: string; liked_at: string }[]>([]);
  const [likingCount, setLikingCount] = useState<number>(0)
  ...
  const handleDisplayLikings = async () => {
    ...
    if (data && data.length > 0) {
      const userDetailsPromises = data.map(async (like) => {
        ...
        return { ...userData, liked_at: like.liked_at }; // Include liked_at in the response
      });
      ...
      const validUserDetails = userDetails.filter(
        (user): user is { id: string; username: string; avatar_url: string; liked_at: string } =>
          user !== null
      );
  
      setLikeDetails(validUserDetails);
    } else {
      setLikeDetails([]); // No likes found
    }
  };
  ...
  <div className="mt-4 mb-48 md:mb-20">
    {likeDetails.length > 0 ? (
      <ul>
        {likeDetails.map((user) => (
          <li key={user.id} className="mb-1 p-2 bg-white flex items-center justify-between">
            ...
            <p className="text-xs text-gray-500">
              {formatDate(user.liked_at)} {/* Add a timestamp if available */}
            </p>
          </li>
        ))}
      </ul>
